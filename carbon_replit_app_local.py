...
def main():
    ...
    st.sidebar.markdown("⚖️ **Vergleichsanalyse** - Multi-Szenario-Vergleich")
        ...
        with st.container():
            ...
            with col_model:
                ...
                if vehicle_type == "Elektrofahrzeug":
                    try:
                        electric_cars = pd.DataFrame(st.session_state.db_manager.get_german_cars(\"electric\"))
                    ...
                    try:
                        diesel_cars = pd.DataFrame(st.session_state.db_manager.get_german_cars(\"diesel\"))
